...
\subsection{O Problema que o Transformer Resolve}\label{subsec:limitacoes_rnn}

Toda arquitetura nasce como resposta a uma limitação da anterior, e o \textit{Transformer}
não é exceção. Compreender qual era essa limitação é o que transforma o mecanismo de
atenção --- apresentado a seguir --- em uma solução, e não em uma escolha arbitrária.

Até 2017, o processamento de texto era dominado pelas redes neurais recorrentes
(\textit{Recurrent Neural Networks}, RNNs). Elas leem a frase palavra por palavra, na
ordem em que aparece, carregando consigo um resumo do que já foi lido: a cada nova
palavra, esse resumo é atualizado. É, em essência, o modo como uma pessoa lê um texto em
voz alta, guardando na memória o que veio antes. Dessa mecânica decorrem os dois
problemas que motivaram o \textit{Transformer}.

O primeiro é o \textbf{esquecimento}. Considere o trecho ``A Inglaterra é minha terra
natal. Passei toda a minha vida lá. Mudei-me para a Espanha há dois dias. Eu falo apenas
uma língua, que é o \_\_\_''. Preencher a lacuna com ``inglês'' exige recuperar uma
palavra que apareceu muito antes. Como o resumo é reescrito a cada passo, a informação
antiga vai se diluindo, e a influência de ``Inglaterra'' chega enfraquecida ao final.
Tecnicamente, o fenômeno se manifesta durante o treinamento como gradiente desaparecente
(\textit{vanishing gradient}): os gradientes são multiplicados repetidamente pelas
matrizes de peso e, quando esses valores são pequenos, encolhem até se aproximarem de zero
\cite{Bengio1994}. É a limitação conhecida como incapacidade de lidar com
\textbf{dependências de longo alcance}. Arquiteturas posteriores, como as redes LSTM
(\textit{Long Short-Term Memory}), atenuaram o esquecimento por meio de mecanismos que
regulam o que deve ser mantido ou descartado \cite{Hochreiter1997}, mas não resolveram o
segundo problema.

O segundo é a \textbf{lentidão}, e é o que mais interessa a este trabalho. Como cada
palavra só pode ser processada depois da anterior, a leitura é obrigatoriamente
sequencial: não há como dividir o trabalho entre vários processadores. Isso desperdiça
justamente a principal característica das unidades de processamento gráfico (GPUs)
modernas, que é executar milhares de operações ao mesmo tempo. A analogia é a de um
supermercado com vinte caixas abertos, mas com uma regra que obrigue os clientes a passar
por um de cada vez: os outros dezenove ficam ociosos, e a fila anda no ritmo de um só.

Foi para atacar essas duas limitações simultaneamente que se desenvolveu o mecanismo de
atenção.
...
parâmetros é cerca de cinco vezes maior do que o de uma ResNet-50 (25 milhões de
parâmetros), arquitetura de referência nos estudos clássicos de poda --- e as variantes
maiores da própria família GPT-2 chegam a 1,5 bilhão de parâmetros, o que ilustra a escala
característica dos modelos baseados em \textit{Transformer}.
