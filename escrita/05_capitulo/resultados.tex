...
derivada (Seção~\ref{sec:res_alocacao}). A
Seção~\ref{sec:res_finetune} submete as configurações mais relevantes ao
\textit{fine-tuning} de recuperação, e a Seção~\ref{sec:res_sintese}
...
constantes: 474,70~MB e 126,58~G em todas as dez configurações, valores
idênticos dígito a dígito. A explicação é direta: a poda não estruturada
substitui pesos por zeros, mas não remove nada da arquitetura. Os tensores
mantêm as mesmas dimensões, e a GPU multiplica os zeros exatamente como
multiplicava os valores originais. O ganho de 61\,\% em parâmetros é contábil,
não físico.

As grandezas medidas empiricamente --- tempo e energia --- comportam-se de
modo ligeiramente diferente das calculadas: exibem uma deriva descendente
pequena e monótona ao longo da varredura, de 54,0 para 51,0~ms e de 1572 para
1432~$\mu$Wh, isto é, 5,6\,\% e 8,9\,\% entre os extremos. Essa deriva é da
ordem da variação observada entre execuções repetidas do mesmo modelo denso
--- que registrou de 40,5 a 54,0~ms e de 1176 a 1572~$\mu$Wh conforme a sessão,
como discutido na Seção~\ref{sec:res_protocolo} --- e não pode ser atribuída à
poda, uma vez que a arquitetura executada é literalmente a mesma em todos os
níveis. Serve, na verdade, como calibração da magnitude do ruído de medição:
qualquer efeito de poda inferior a cerca de 10\,\% em tempo ou energia está
abaixo do limiar de detecção deste protocolo. Os 55,5\,\% de redução de tempo
que a poda estruturada registra na próxima seção estão uma ordem de grandeza
acima desse limiar.
...
poda não estruturada não entrega ganho de eficiência algum.
...
$\beta$, escalada por um fator único obtido por bisseção de modo que o
orçamento global seja respeitado, e limitada pelo teto $r_{\max} = 0{,}95$.
Interessa aqui apenas o que ela produz na prática.

A Tabela~\ref{tab:alocacao_50} mostra as taxas que a
Equação~\ref{eq:alocacao_dev} produziu para um orçamento global de 50\,\% com
...
Equação~\ref{eq:alocacao_dev} para orçamento global de 50\,\% e $\beta = 1$
...
O perfil apresentado até aqui foi levantado a uma taxa-sonda de 50\,\%. Como a
...
réplica independente da fase 1. A Tabela~\ref{tab:sonda} confronta os dois
levantamentos.

\begin{table}[htb]
\centering
\caption{Perfis de sensibilidade levantados a duas taxas-sonda. Os valores são
os $\Delta_{\ell}$ da Equação~\ref{eq:delta_perfil}; entre parênteses, o posto
da camada na ordenação de sensibilidade (1 = mais sensível).}
\label{tab:sonda}
\small
\begin{tabular}{c cc cc}
\toprule
& \multicolumn{2}{c}{\textbf{Magnitude}} & \multicolumn{2}{c}{\textbf{Estruturada}} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
\textbf{Camada} & \textbf{Sonda 30\,\%} & \textbf{Sonda 50\,\%}
                & \textbf{Sonda 30\,\%} & \textbf{Sonda 50\,\%} \\
\midrule
 0 & 0,0414 (1)  & 0,3899 (1)  & 3,4235 (1)  & 4,0088 (1) \\
 1 & 0,0067 (12) & 0,0346 (12) & 0,1593 (3)  & 0,0944 (11) \\
 2 & 0,0121 (7)  & 0,0929 (4)  & 0,1302 (7)  & 0,1726 (5) \\
 3 & 0,0074 (11) & 0,0433 (11) & 0,1426 (5)  & 0,1832 (4) \\
 4 & 0,0390 (2)  & 0,2023 (2)  & 0,1560 (4)  & 0,2184 (3) \\
 5 & 0,0346 (3)  & 0,1298 (3)  & 0,0929 (8)  & 0,1688 (6) \\
 6 & 0,0164 (4)  & 0,0735 (5)  & 0,1320 (6)  & 0,1383 (7) \\
 7 & 0,0077 (10) & 0,0490 (10) & 0,0526 (12) & 0,1176 (10) \\
 8 & 0,0107 (8)  & 0,0549 (7)  & 0,0650 (10) & 0,1225 (9) \\
 9 & 0,0137 (5)  & 0,0611 (6)  & 0,0596 (11) & 0,0934 (12) \\
10 & 0,0078 (9)  & 0,0535 (9)  & 0,0885 (9)  & 0,1378 (8) \\
11 & 0,0132 (6)  & 0,0535 (8)  & 0,5387 (2)  & 0,6635 (2) \\
\bottomrule
\end{tabular}\\
...
Registre-se que o perfil de 30\,\% foi mantido em arquivo separado e não
alimentou nenhuma das alocações reportadas: todas as varreduras da
Seção~\ref{sec:res_alocacao} usam exclusivamente o perfil de 50\,\%. A
réplica cumpre aqui o papel de verificação, não de insumo.
...
camada produzidas pela Equação~\ref{eq:alocacao_dev} foram aplicadas nos mesmos
dez níveis de orçamento das varreduras anteriores, para as duas estratégias, e
avaliadas com o mesmo vetor de métricas. O experimento foi replicado com três
valores do expoente $\beta$ --- 1,0, 0,5 e 0,0 ---, o que fornece a ablação da
função de alocação sobre um mesmo perfil. O caso $\beta = 0$ não é uma
alocação candidata, e sim um controle: nele a regra atribui a mesma taxa a
todas as camadas e deve, por construção, reproduzir a poda uniforme
...
Na poda estruturada a coincidência é \textbf{exata}: nos dez níveis de
orçamento, a alocação com $\beta = 0$ reproduz a perplexity da varredura
...
que a alocação guiada obtém a mais, nas seções anteriores, vem do perfil e não
de alguma diferença espúria de procedimento.
...
90\,\%. A alocação por sensibilidade com $\beta = 0$ não é, portanto, a poda
...
Esta seção fecha a lacuna descrita na Seção~\ref{sec:finetuning}, submetendo
sete configurações representativas ao \textit{fine-tuning} de recuperação e
medindo a perplexity antes e depois do retreinamento. O orçamento de treino é
idêntico para todas: uma época sobre o conjunto de treino do WikiText-2, 297
...
\begin{table}[htb]
\centering
\caption{\textit{Fine-tuning} de recuperação (uma época, 297 passos). A última
coluna expressa a perplexity pós-retreinamento como múltiplo da perplexity do
modelo denso retreinado, e não do modelo denso original.}
\label{tab:finetune}
\small
\begin{tabular}{l c rr c}
\toprule
\textbf{Configuração} & \textbf{Esp. real} & \textbf{PPL \textit{one-shot}} & \textbf{PPL pós-FT} & \textbf{Razão} \\
\midrule
Denso (controle)            & ---      &   25,17 &  18,68 & 1,00 \\
\midrule
Magnitude 30\,\% (uniforme) & 30,00\,\% &   36,42 &  20,54 & 1,10 \\
Magnitude 50\,\% (uniforme) & 50,00\,\% &  473,28 &  30,27 & 1,62 \\
\midrule
Estruturada 10\,\% (global) &  9,90\,\% &  356,20 &  25,33 & 1,36 \\
Sensibilidade 10\,\% ($\beta = 1$) &  9,20\,\% &  124,29 &  25,96 & 1,39 \\
\midrule
Estruturada 30\,\% (global) & 29,95\,\% & 2413,52 & 104,78 & 5,61 \\
Sensibilidade 30\,\% ($\beta = 1$) & 29,01\,\% &  724,53 &  56,08 & 3,00 \\
\bottomrule
\end{tabular}
\end{table}
...
modelo denso, submetido ao mesmo orçamento de treino, tem sua perplexity
...
o aproxima da distribuição do corpus de avaliação.
...
fato é --- 10\,\% pior que o denso equivalente. Toda a coluna de razão da
Tabela~\ref{tab:finetune} adota essa referência, e é a única leitura que
separa recuperação do dano da poda de mera adaptação ao corpus.
...
após um orçamento modesto de retreinamento, recuperáveis a patamares
...
retreinamento entra no orçamento.
...
Seção~\ref{sec:res_magnitude}, não reduz tempo, energia nem memória. As
configurações estruturadas a 30\,\%, essas sim, registram 40,82~ms e
1,285~mWh por janela contra 50,21~ms e 1,588~mWh do denso retreinado --- uma
redução de aproximadamente 19\,\% em ambos.

\subsection{O \textit{Fine-tuning} Corrói a Vantagem da Alocação
Guiada}\label{subsec:ft_corroi}

O achado mais desconfortável desta varredura diz respeito diretamente à
contribuição central do trabalho. As duas últimas linhas de cada bloco da
Tabela~\ref{tab:finetune} confrontam a alocação guiada por sensibilidade com a
alocação global, nos mesmos dois orçamentos, antes e depois do retreinamento.

Em 10\,\% de estruturas removidas, a vantagem \textbf{desaparece}. Em regime
\textit{one-shot}, a alocação guiada registrava perplexity 124,29 contra
356,20 da alocação global, uma vantagem de 2,87 vezes. Após o retreinamento, a
...
A interpretação é que a alocação por sensibilidade otimiza uma quantidade
específica --- o dano imediato causado pela poda --- e o retreinamento repara
justamente o tipo de dano que ela evita. Quanto maior o orçamento de
recuperação, menor a importância de haver podado bem. Sob essa leitura, o
perfil de sensibilidade é mais valioso quanto mais escasso for o orçamento de
...
orçamentos de compressão agressivos, a alocação guiada continua entregando
perplexity aproximadamente duas vezes menor ao mesmo custo computacional
--- 40,28~ms e 1,307~mWh contra 40,82~ms e 1,285~mWh da alocação global em
30\,\%, valores equivalentes dentro da variação de medição. Em orçamentos
leves, seguidos de retreinamento, a alocação ingênua é suficiente.
...
reais das configurações guiadas por sensibilidade, por sua vez, ficam
...
indivisíveis. A diferença favorece marginalmente a alocação guiada e está
registrada aqui por transparência, ainda que inferior a um ponto percentual.

Quanto ao custo, o retreinamento consome 7,33~mWh no modelo denso, contra
1,59~mWh de uma avaliação completa do mesmo modelo --- aproximadamente 4,6
...
configuração estruturada a 30\,\% economiza 0,303~mWh por avaliação completa
do conjunto de teste (1,285 contra 1,588~mWh), de modo que o custo do próprio
...
A limitação principal desta varredura é o orçamento de retreinamento: uma
única época. Não é possível afirmar, a partir destes dados, se a erosão da
vantagem da alocação guiada se estabiliza no patamar observado em 30\,\% ou se
prossegue com mais épocas até desaparecer também nesse orçamento. Trata-se da
...
energia ou memória. O escopo da conclusão é delimitado por três fronteiras: a
saturação da alocação em orçamentos acima de 60\,\%; o fato de que o perfil
de sensibilidade precisa ser levantado separadamente para cada estratégia de
poda, por não ser transferível entre elas; e, sobretudo, o orçamento de
retreinamento disponível.

Esta última fronteira merece registro explícito, por ser a que mais qualifica
a contribuição. O \textit{fine-tuning} de recuperação
(Seção~\ref{sec:res_finetune}) repara justamente o tipo de dano que a
alocação guiada evita, e por isso corrói sua vantagem: em 10\,\% de
estruturas removidas a vantagem desaparece após uma época de retreinamento, e
em 30\,\% ela se reduz de um fator de 3,33 para 1,87. A alocação por
sensibilidade é, portanto, uma contribuição cujo valor cresce com a severidade
da compressão e decresce com o orçamento de recuperação disponível --- útil
